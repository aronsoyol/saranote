...
var toc = (function () {
  ...
  var $_gieo4uqyjepbvcze = { create: create };

  var tocId = $_gieo4uqyjepbvcze.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_ccn6ciqxjepbvczc.getTocClass(editor);
    var headerTag = $_ccn6ciqxjepbvczc.getTocHeader(editor);
    var selector = generateSelector($_ccn6ciqxjepbvczc.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_ccn6ciqxjepbvczc.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_ccn6ciqxjepbvczc.getTocHeader(editor), I18n.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_akp1kxqtjepbvcz6.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_akp1kxqtjepbvcz6.updateToc(editor);
    });
  };
  var $_d8t0cqqsjepbvcz4 = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_ccn6ciqxjepbvczc.getTocClass(editor);
  ...
  var $_8k69hxqzjepbvczf = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_akp1kxqtjepbvcz6.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_ccn6ciqxjepbvczc.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_fr0pc3r0jepbvczg = { register: register$1 };

  PluginManager.add('toc', function (editor) {
    $_d8t0cqqsjepbvcz4.register(editor);
    $_fr0pc3r0jepbvczg.register(editor);
    $_8k69hxqzjepbvczf.setup(editor);
  });
  function Plugin () {
  }
