...
var searchreplace = (function () {
  ...
  var $_14f8k4isjepbvawq = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_14f8k4isjepbvawq.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_83z7ryiqjepbvawj.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_83z7ryiqjepbvawj.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_83z7ryiqjepbvawj.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_83z7ryiqjepbvawj.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_83z7ryiqjepbvawj.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_83z7ryiqjepbvawj.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_83z7ryiqjepbvawj.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_83z7ryiqjepbvawj.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_83z7ryiqjepbvawj.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_83z7ryiqjepbvawj.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_83z7ryiqjepbvawj.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_83z7ryiqjepbvawj.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_83z7ryiqjepbvawj.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_83z7ryiqjepbvawj.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_83z7ryiqjepbvawj.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_cc6wf5iujepbvawy = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_cc6wf5iujepbvawy.open(editor, currentIndexState);
    });
  };
  var $_dmlcdaitjepbvaww = { register: register };
  ...
  var $_2wraw3ivjepbvax2 = { register: register$1 };

  PluginManager.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_dmlcdaitjepbvaww.register(editor, currentIndexState);
    $_2wraw3ivjepbvax2.register(editor, currentIndexState);
    return $_enksv4ipjepbvawf.get(editor, currentIndexState);
  });
  function Plugin () {
  }
