...
var textpattern = (function () {
  ...
  var $_3g84k5qhjepbvcy4 = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_1miwfzqpjepbvcyg.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_1miwfzqpjepbvcyg.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_1miwfzqpjepbvcyg.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_emngvtqmjepbvcyc.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_emngvtqmjepbvcyc.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_emngvtqmjepbvcyc.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !VK.modifierPressed(e)) {
        $_6p3isaqljepbvcy9.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_6p3isaqljepbvcy9.checkKeyCode(keyCodes, e)) {
        $_6p3isaqljepbvcy9.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_6p3isaqljepbvcy9.checkCharCode(charCodes, e)) {
  ...
  var $_e0h626qijepbvcy6 = { setup: setup };

  PluginManager.add('textpattern', function (editor) {
    var patternsState = Cell($_3g84k5qhjepbvcy4.getPatterns(editor.settings));
    $_e0h626qijepbvcy6.setup(editor, patternsState);
    return $_5okzw8qgjepbvcy3.get(patternsState);
  });
  function Plugin () {
  }
